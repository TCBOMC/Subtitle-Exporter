...
class SubtitleExtractorApp:
    def __init__(self, root):
        ...
        self.root.minsize(width=550, height=200)
    ...
    def create_widgets(self):
        ...
        self.import_btn.grid(row=0, column=0, padx=(10, 5))
        ...
        ttk.Label(self.subfmt_frame, text="格式:").grid(row=0, column=0, padx=(5, 5), sticky="e")
        ...
        self.subfmt_cb.grid(row=0, column=1, padx=(0, 5), sticky="e")
        ...
        self.ass_fix_cb.grid(row=0, column=4, padx=(5, 0), sticky="w")
        ...
        ttk.Label(self.font_mode_frame, text="").grid(row=0, column=0, padx=(5, 2), sticky="e")
        ...
        self.font_mode_cb.grid(row=0, column=1, padx=(0, 5), sticky="w")

        self.extract_btn = ttk.Button(button_frame, text="提取字幕", command=self.extract_subtitles_clicked)
        self.extract_btn.grid(row=0, column=6, padx=(0, 17), sticky="e")
        ...
        self.tree.column("width", width=50, anchor="center", stretch=False)
        self.tree.column("height", width=50, anchor="center", stretch=False)
        self.tree.column("fps", width=50, anchor="center", stretch=False)
    ...
    def refresh_tree(self):
        ...
        for file_tuple in self.files:
            ...
            chk = "☑" if checked else "☐"
            self.tree.insert("", tk.END, iid=fullpath, values=(chk, filename, width, height, fps))
    ...
    def replace_font_name_complete(self,font_path, old_name, new_name, output_path=None):
        """
        替换字体中所有包含 old_name 的名称记录。
        - PostScript 名称(nameID=6) → ASCII标准化
        - 对支持 TTF name 的字体：
            英文名称 → 拼音化
            中文名称 → 保留
        - 对不支持 TTF name 的字体：
        ...
        try:
            ...
            # 简单检测字体是否支持 TTF name
            # 一般 CID-keyed OTF 的 'CFF ' 表里只有 PostScript 字体，不支持 Windows Unicode name
                ...
                if decoded and old_name in decoded:
                    ...
                    if record.nameID == 6:
                        ...
                        if supports_ttf_name:
                            ...
                            # 不支持 TTF name → 英文也使用中文
                            record.string = chinese_name.encode(
                                'utf-16be') if record.platformID != 1 else chinese_name.encode('mac_roman')

            # 确保中文记录存在（nameID 1/4/16/17）
            ...
            try:
                if len(font_files) == 1:
                    ...
                    if self._run_fontforge_script(merge_script):
                        success_count += 1
                        self.log(f"✅ 合并字体: {base_name} ({len(font_files)}个文件)")
                    else:
...
'''
    ...
    def toggle_all_selection(self):
        ...
        for idx, file_tuple in enumerate(self.files):
            ...
            filename = file_info.get("filename")
            # 更新 checked 状态
            self.files[idx] = (fullpath, filename, self.header_checked)
            self.tree.set(fullpath, "Check", "☑" if self.header_checked else "☐")
        self.update_header_checkbox()
    ...
    def on_tree_click(self, event):
        ...
        for i, file_tuple in enumerate(self.files):
            ...
            if fullpath == item:
                ...
                self.files[i] = (fullpath, filename, new_checked)
                self.tree.set(item, "Check", "☑" if new_checked else "☐")
                break
    ...
    def on_files_dropped(self, event):
        ...
        for file_path in final_files:
            file_path = file_path.strip("'")  # 移除路径中的引号
            if os.path.isfile(file_path) and not any(existing_file[0] == file_path for existing_file in self.files):
                ...
                try:
                    info = self.silent_ffmpeg_probe(file_path)
                    probe_info = info  # 保存完整视频信息
                    for stream in info.get("streams", []):
                        if stream.get("codec_type") == "video":
                            width = stream.get("width", "")
                ...
                self.files.append(file_tuple)
                self.original_files.append((file_path, file_name))
            else:
                if not os.path.isfile(file_path):
                    print(f"无效的文件路径: {file_path}")  # 如果路径无效，输出提示
                else:
                    print(f"文件已存在: {file_path}")  # 如果文件已经在列表中，输出提示
